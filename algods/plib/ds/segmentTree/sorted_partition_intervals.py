...
class ChthollyTree:
    """ 
        ...
        example: 
            ChthollyTree([10,20,30])
            self.nodes is {0:10, 1:20, 2:30}
                only store left endpoints, it's enough to represent all the three intervals
        app: 
        ...
        test: 
            previous version:@cf#896C(TLE) https://codeforces.com/contest/896/submission/148760838
            @lc#2213
    ...
    def union_at(self, itv_l):
        ...
        assert: 0<=x<n, there is an interval whose leftendpoint is x
        """
        itvs = self.itvs
